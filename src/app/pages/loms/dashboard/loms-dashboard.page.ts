...
export class TradeDashboardPageComponent implements OnInit {
    ...
    },
    {
      ...
      history: [
        ...
        },
        {
          ...
          time: '10:45',
          actor: 'Branch Maker',
          comment: 'Submitted to risk queue'
        }
      ],
      missingDocs: 0,
      openQueries: 0,
    ...
    },
    {
      ...
      history: [
        ...
        },
        {
          ...
          time: '13:20',
          actor: 'System',
          comment: 'Waiting for allocation'
        }
      ],
      missingDocs: 0,
      openQueries: 1,
  ...
  clearAllFilters(): void {
    ...
    this.filterGroups.forEach(group => {
      group.options.forEach(option => {
        if (group.type === 'radio') {
          option.selected = option.key === 'all';
        } else {
  ...
  applyFilters(): void {
    ...
    const selectedProducts =
      productGroup?.options.filter(o => o.selected).map(o => o.label) ?? [];
    if (selectedProducts.length) {
      rows = rows.filter(r => selectedProducts.includes(r.productCategory));
    }

    const productNameGroup = this.filterGroups.find(g => g.key === 'productName');
    const selectedProductNames =
      productNameGroup?.options.filter(o => o.selected).map(o => o.label) ?? [];
    if (selectedProductNames.length) {
      rows = rows.filter(r => selectedProductNames.includes(r.productName));
    }

    const statusGroup = this.filterGroups.find(g => g.key === 'status');
    const selectedStatuses =
      statusGroup?.options.filter(o => o.selected).map(o => o.label) ?? [];
    if (selectedStatuses.length) {
      rows = rows.filter(r => selectedStatuses.includes(r.statusDisplay));
    }

    const stageGroup = this.filterGroups.find(g => g.key === 'stage');
    const selectedStages =
      stageGroup?.options.filter(o => o.selected).map(o => o.label) ?? [];
    if (selectedStages.length) {
      rows = rows.filter(r => selectedStages.includes(r.stage));
    }
    ...
    if (selectedAssignee && selectedAssignee.key !== 'all') {
      if (selectedAssignee.key === 'me') {
        rows = rows.filter(r => r.assigneeName === 'Me');
      } else if (selectedAssignee.key === 'unassigned') {
        rows = rows.filter(r => r.assigneeName === 'Unassigned');
      }
    }

    const slaGroup = this.filterGroups.find(g => g.key === 'sla');
    const selectedSla =
      slaGroup?.options.filter(o => o.selected).map(o => o.label) ?? [];
    ...
    if (term) {
      rows = rows.filter(row => {
        const combined = `${row.id} ${row.productCategory} ${row.productName} ${row.statusDisplay} ${row.customerLabel} ${row.customerId} ${row.source} ${row.assigneeRole} ${row.assigneeName}`.toLowerCase();
  ...
  initializeFilterGroups(): void {
    const productCounts = this.buildCountMap(
      this.unifiedRows.map(r => r.productCategory)
    );
    const productNameCounts = this.buildCountMap(
      this.unifiedRows.map(r => r.productName)
    );
    const statusCounts = this.buildCountMap(
      this.unifiedRows.map(r => r.statusDisplay)
    );
    const stageCounts = this.buildCountMap(this.unifiedRows.map(r => r.stage));
    const slaCounts = this.buildCountMap(
      this.unifiedRows.map(r => (r.slaBreached ? 'Overdue' : 'Due Today'))
    );
    ...
    const assigneeOptions: FilterOption[] = [
      {
        key: 'all',
        label: 'All',
        count: this.unifiedRows.length,
        selected: true
      },
      {
        key: 'me',
        label: 'Assigned to Me',
        count: this.unifiedRows.filter(r => r.assigneeName === 'Me').length,
        selected: false
      },
      {
        key: 'unassigned',
        label: 'Unassigned',
        count: this.unifiedRows.filter(r => r.assigneeName === 'Unassigned')
          .length,
        selected: false
      }
    ];
    ...
    this.filterGroups = [
      {
        ...
        label: 'PRODUCT',
        type: 'checkbox',
        options: this.mapCountMapToOptions(productCounts)
      },
      {
        key: 'productName',
        label: 'PRODUCT NAME',
        type: 'checkbox',
        options: this.mapCountMapToOptions(productNameCounts)
      },
      {
        key: 'status',
        label: 'STATUS',
        type: 'checkbox',
        options: this.mapCountMapToOptions(statusCounts)
      },
      {
        key: 'stage',
        label: 'STAGE',
        type: 'checkbox',
        options: this.mapCountMapToOptions(stageCounts)
      },
      {
        key: 'assignee',
        label: 'ASSIGNEE',
  ...
  removeChip(chip: FilterChip): void {
    if (chip.type === 'view') {
      ...
      const [groupKey, optionKey] = chip.key.split(':') as [
        FilterGroupKey,
        string
      ];
      const group = this.filterGroups.find(g => g.key === groupKey);
