...
interface FinancialBankAccount {
...
interface FinancialApplicationForm {
  informationType: 'account' | 'liability';
  employmentType: string;
  bankAccounts: FinancialBankAccount[];
}

type FinancialApplicationStepStatus = 'completed' | 'in-progress' | 'pending';

@Component({
  selector: 'app-loms-financial-application-page',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './financial-application.page.html'
})
export class LomsFinancialApplicationPageComponent {
  ...
  private readonly formStorageKey = 'lomsFinancialForm';
  ...
  bankNameOptions = [
    ...
    { label: 'BRAC Bank', value: 'brac_bank' }
  ];

  accountTypeOptions = [
    { label: 'Current Account', value: 'current_account' },
    { label: 'Savings Account', value: 'savings_account' },
    { label: 'Salary Account', value: 'salary_account' }
  ];

  form: FinancialApplicationForm = {
  ...
  lastSavedDraft: FinancialApplicationForm | null = null;
  lastSubmittedApplication: FinancialApplicationForm | null = null;
  ...
  getStepStatus(index: number): FinancialApplicationStepStatus {
  ...
  exportApplicationData(): FinancialApplicationForm {
    return this.form;
  }
  ...
  private loadFormFromStorage(): void {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as Partial<FinancialApplicationForm>;
      this.form = {
        ...this.form,
        ...parsed
      };
